...
class EventOutboxTests(unittest.TestCase):
    ...
    def test_outbox_message_is_not_claimable_before_activation(self) -> None:
        outbox = InMemoryOutbox()
        tenant = self.principal("tenant-a")
        event = CloudEvent(
        ...
        activates_at = datetime(2100, 1, 1, tzinfo=UTC)

        message = outbox.add(
            tenant,
            event,
            idempotency_key="future-notification",
            next_attempt_at=activates_at,
        )

        self.assertEqual(message.next_attempt_at, activates_at)
        self.assertEqual(outbox.claim(tenant), [])
